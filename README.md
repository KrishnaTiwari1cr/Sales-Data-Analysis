# 📊 Business Performance Analysis

A comprehensive sales and financial analysis project built using Excel. This project demonstrates end-to-end data analysis including data cleaning, transformation, dashboarding, and actionable business insights.

---

## 📌 Sales Analysis

### 🎯 Project Objectives
- Analyze customer trends using [Customer Performance Report](https://github.com/KrishnaTiwari1cr/Sales-Data-Analysis/blob/main/Customer%20Performance%20Report.pdf)
- Compare targets vs actuals via [Market Performance vs Target](https://github.com/KrishnaTiwari1cr/Sales-Data-Analysis/blob/main/Market%20Performance%20vs%20Target.pdf)
- Identify top-performing products using [Top 10 Products](https://github.com/KrishnaTiwari1cr/Sales-Data-Analysis/blob/main/Top%2010%20Products.pdf)

### 📈 Key Insights
- Identified revenue-driving customers and markets  
- Highlighted underperforming regions vs targets  
- Discovered top-performing products contributing to maximum revenue  

### 💡 Business Value
- Improves sales strategy and targeting  
- Enables data-driven decision-making  
- Enhances customer segmentation  

---

## 💰 Financial Analysis

### 🎯 Project Objectives
- Analyze Profit & Loss Reports by:
  - [Year](https://github.com/KrishnaTiwari1cr/Sales-Data-Analysis/blob/main/Profit%20%26%20Loss%20report%20by%20Year.pdf)
  - [Month](https://github.com/KrishnaTiwari1cr/Sales-Data-Analysis/blob/main/Profit%20%26%20Loss%20report%20by%20month.pdf)
  - [Market](https://github.com/KrishnaTiwari1cr/Sales-Data-Analysis/blob/main/Profit%20%26%20Loss%20report%20by%20Market.pdf)
- Evaluate profitability using [Gross Margin Analysis](https://github.com/KrishnaTiwari1cr/Sales-Data-Analysis/blob/main/Gross%20margin%20%25.pdf)

### 📊 Key Insights
- Identified profit trends across time and regions  
- Highlighted high and low margin segments  
- Detected cost inefficiencies  

### 💡 Business Value
- Supports financial planning & forecasting  
- Improves profitability analysis  
- Assists strategic decision-making  

---

## 🔄 Project Workflow
1. Data Cleaning (Power Query)  
2. Data Transformation  
3. Data Analysis (Excel)  
4. Dashboard & Report Creation  
5. Insights Generation  

---

## 🛠️ Technical Skills
- Excel (Advanced)  
- Power Query (ETL)  
- Pivot Tables  
- Data Visualization  
- Financial Analysis  

---

## 🧠 Analytical Skills
- Trend Analysis  
- Forecasting  
- Variance Analysis  
- KPI Understanding  

---

## 💡 Business Recommendations
- Focus on high-performing products and markets  
- Improve performance in low-revenue regions  
- Optimize pricing and cost strategies for better margins  

---

## 🚀 Project Highlights
- End-to-end Excel-based analysis project  
- Business-focused insights  
- Structured and professional reporting  

---

## 👤 Author
**Krishna Tiwari**  
Aspiring Data Analyst  
